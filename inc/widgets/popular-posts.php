...
function kantipur_blog_popular_posts() {
	register_widget( 'Kantipur_Blog_Popular_Posts' );
}
add_action( 'widgets_init', 'kantipur_blog_popular_posts' );

class Kantipur_Blog_Popular_Posts extends WP_Widget { 

	function __construct() {
		global $control_ops;
		$widget_ops = array(
		  'classname'   => 'widget_popular_posts',
		  'description' => esc_html__( 'Add Widget to Display Popular Posts.', 'kantipur-blog' )
		);
		parent::__construct( 'Kantipur_Blog_Popular_Posts',esc_html__( 'Popular Posts Section', 'kantipur-blog' ), $widget_ops, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, 
			array( 
			  'title'			=> esc_html__( 'Popular this week', 'kantipur-blog' ),	
			  'category'       	=> '', 
			  'number'          => 3, 
			) 
		);
		$title     			= isset( $instance['title'] ) ? esc_html( $instance['title'] ) : esc_html__( 'Popular this week', 'kantipur-blog' );
